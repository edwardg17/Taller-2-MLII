...
class kmedoids:
    ...
    def rand_centroids(self, X, nclusters):
        ...
        rows, colms = X.shape
        #Initial medoid vector
        medoids =np.empty([nclusters,colms])
        ...
        for n, part in enumerate(partitions):
            ran_index = np.random.randint(part.shape[0])
            medoids[n] = part[ran_index]
        
        """
        Return
        medoids: medoids vector, whit dimension k x D (k: number of clusters)
        """
        return medoids

    def assignment_vector(self,X,medoids):
        """
        Inputs
        X: array like whit dimension M x D, where D is the dimension
        medoids: initial medoids vector, whit dimension k x D (k: number of clusters)
        """
        #Dimension of the array
        rows, colms = X.shape   
        k_index     = np.empty([rows])
        eu_distance = self.distance(X, medoids)
    ...
    def update_centroids(self,X,medoids,k_index):
        """
        Inputs
        X: array like whit dimension M x D, where D is the dimension
        medoids: old centroids medoids, whit dimension k x D (k: number of clusters)
        k_index: index of the minimum distance for each row and assignment for the data
        """
        #Dimension of the array
        rows, colms = medoids.shape
        new_medoid  = np.empty((rows,colms))
        for i in range(rows):
            new_medoid[i] = np.median(X[k_index == i], axis = 0)
        """
        Return
        new_medoid: new medoid vector, whit dimension k x D (k: number of clusters)
        """    
        return new_medoid

    def losses(self,X,medoid,k_index):
        """
        Inputs
        X: array like whit dimension M x D, where D is the dimension
        medoid: medoid vector, whit dimension k x D (k: number of clusters)
        k_index: index of the minimum distance for each row and assignment for the data
        """    
        #Obtain euclidian distance 
        eu_distance = self.distance(X,medoid)
        #initial loss value
        loss     = 0.0
        #Data dimension
        rows,colms = X.shape
    ...
    def fit(self, X, nclusters, max_iter=100, abs_tol=1e-16, rel_tol=1e-16, printable=False):
        ...
        """
        medoids = self.rand_centroids(X, nclusters)

        for iter in range(max_iter):
            k_index     = self.assignment_vector(X,medoids)
            medoids     = self.update_centroids(X,medoids,k_index)
            loss        = self.losses(X,medoids,k_index)
            K = medoids.shape[0]
            if iter:
